            ...
            if (exchange.getRequestMethod().toUpperCase().equals("GET")) {
                ...
                if (reqHeaders.containsKey("Authorization")) {
                    ...
                    if (username != null) {
                        ...
                        EventRequest request = new EventRequest(username);
                        if (strings.length > 2) {
                        ...
                        {
                            EventServiceArray service = new EventServiceArray(request);
                            EventResultArray result = service.processEventArray();
                            if (result.success) {
                                exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0);
                                OutputStream respBody = exchange.getResponseBody();
                                WriteString.writeString(GsonSerializer.toJson(result), respBody);
